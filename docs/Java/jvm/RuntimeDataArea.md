...
- 存储在JVM中的Java对象可以被划分为两类：
  ...
  - 另外一类对象的生命周期非常长在某些极端的情况下还能够与JVM的生命周期保持一致
- Java堆区进一步细分，可以划分为新生代（Young Generation）和老年代（Old Generation）
- 其中新生代又可以划分为Eden空间、Survivor0空间和Survivor1空间（或S0、S1空间，或from、to空间）
- 新生代与老年代的空间占比默认为1:2，新生代占1，老年代占2，新生代占整个堆的1/3，可以设置修改``-XX:NewRatio=n``，表示新生代占1，老年代占n，新生代占整个堆的1/(n+1)
- 而在新生代中，Eden区与两个Survivor区默认占比为8:1:1，与上述同理，同样的可以通过``-XX:SurvivorRatio=n``来调整这个比例为n:1:1。但是在JVM中存在``-XX:-UseAdaptiveSizePolicy``，这是一个自适应的内存分配策略，所以在默认情况下并不是8:1:1的结果，而看到的是6:1:1，此时显式的设置``-XX:SurvivorRatio=8``的话，就能得出8:1:1的情况了
- **几乎所有的对象**都是在Eden空间被new出来分配的
- 绝大部分的对象的销毁是在新生代中进行的。
